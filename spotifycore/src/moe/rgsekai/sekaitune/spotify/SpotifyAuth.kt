...
object SpotifyAuth {
    ...
    private val nuanceMutex = kotlinx.coroutines.sync.Mutex()
    ...
    private suspend fun fetchNuance(): Nuance =
        withContext(Dispatchers.IO) {
            nuanceMutex.withLock {
                ...
                try {
                    // Note: This relies on a third-party-maintained public Gist (sonic-liberation/Spotube) with no SLA.
                    // If Spotify auth fails unexpectedly, verify this Gist first; fallback or replacement may be needed if abandoned.
