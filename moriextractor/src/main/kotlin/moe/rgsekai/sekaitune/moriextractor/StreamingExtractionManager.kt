    ...
    suspend fun extractAudioUrl(
        ...
        withContext(Dispatchers.IO) {
            ...
            if (normalizedVideoUrl.isBlank()) {
                throw SekaiTuneExtractorException("Video URL is missing")
            }
            val normalizedCookies = cookies?.trim()?.takeIf { it.isNotBlank() }

            val token = bearerToken.trim()
            if (token.isBlank()) {
                throw SekaiTuneExtractorException("SekaiTune Extractor token is missing")
            }
            ...
            try {
                ...
                val response =
                    try {
                        ...
                        throw SekaiTuneExtractorException(
                            "SekaiTune Extractor returned invalid JSON",
                            serialization,
                        )
                    }
                    ...
                    val message =
                        response.error
                            ...
                            ?: if (response.success && !response.audioUrl.isNullOrBlank()) {
                                "SekaiTune Extractor returned raw audio_url but no stream_url. Upload/run the stream proxy main.py on the backend."
                            } else {
                                "SekaiTune Extractor returned no playable proxy stream"
                            }
                    _extractorState.value = ExtractorState.Error(message)
                    throw SekaiTuneExtractorException(message)
                }
                ...
                val exception =
                    throwable as? SekaiTuneExtractorException
                        ?: SekaiTuneExtractorException("SekaiTune Extractor request failed", throwable)
...
class SekaiTuneExtractorException(
    message: String,
